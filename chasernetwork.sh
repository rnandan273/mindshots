...
function printHelp() {
  ...
  echo "      - 'chaserCC' - deploy the fabcar chaincode on the channel"
  ...
  echo "  network.sh chaserCC -l -v -r -d -verbose"
  echo
  ...
  echo "  network.sh chaserCC -l javascript"
}
...
function chaserCC() {
  ...
  scripts/chaserCC.sh $CHANNEL_NAME $CC_SRC_LANGUAGE $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
